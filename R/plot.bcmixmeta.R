#' Generic plot function for bcmixmeta object.
#'
...
#'
#' @param x The object generated by the bcmixmeta function (the BC-BNP model output).
...
#' - "mu_posterior": Plots the posterior of the overall mean (mu.0).
#' - "tau_posterior": Plots the posterior of the between-study standard deviation (sd.0).
...
plot.bcmixmeta = function(x,
  ...
  if (plot.type == "predictive") {
    ...
    mu_samples = x$BUGSoutput$sims.list$mu.0[1:n.samples]
    plot_data_mu = data.frame(effect = as.vector(mu_samples))

    # Calculate default x.lim if not specified
    if (is.null(x.lim)) {
      # Calculate quartiles and IQR from the posterior samples
      q1 = quantile(plot_data_mu$effect, 0.25, na.rm = TRUE)
      q3 = quantile(plot_data_mu$effect, 0.75, na.rm = TRUE)
      iqr = q3 - q1
      # Define limits based on the IQR rule (Q1 - 4*IQR, Q3 + 4*IQR)
      lower_lim = q1 - 4 * iqr
      upper_lim = q3 + 4 * iqr
      x.lim = c(lower_lim, upper_lim)
    }
    ...
    tau_samples = x$BUGSoutput$sims.list$sd.0[1:n.samples]
    ...
    if (is.null(x.lim)) {
      ...
      iqr = q3 - q1
      # Define limits based on the IQR rule (Q1 - 4*IQR, Q3 + 4*IQR)
      # sd.0 should not be negative, so the lower bound is constrained
